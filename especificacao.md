# Sistema Runner — Especificação do Trabalho Prático
...
6. **Artefatos executáveis**
   - Binários pré-compilados para as três plataformas suportadas:
     ...
     - `assinatura-<versão>-linux-amd64` (Linux)
     - `assinatura-<versão>-macos-amd64` (macOS)
     - `simulador-<versão>-windows-amd64.exe` (Windows)
     - `simulador-<versão>-linux-amd64` (Linux)
     - `simulador-<versão>-macos-amd64` (macOS)
   - Distribuídos via **GitHub Releases**
   - Cada release deve conter assinatura dos artefatos publicados, conforme a seção abaixo.
   - Versionamento SemVer (ex.: `v1.0.0`)
...
A assinatura dos artefatos **DEVE ser realizada automaticamente pelo pipeline de CI/CD** durante a criação da release.
